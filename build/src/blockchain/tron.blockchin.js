...
class tronBlockChain {
    ...
    privateKey;
    tronWeb;
    constructor(rpcURL = "https://api.trongrid.io", privateKey) {
        this.rpcURL = rpcURL;
     ...
     * @returns
     */
    async sendTRX(fromAddress, toAddress, amountTRX, privateKey) {
        // Convert TRX to SUN
        ...
        try {
            ...
            const transaction = await this.tronWeb.transactionBuilder.sendTrx(toAddress, amountInSUN
            //   tronWeb.defaultAddress.base58
            );
            const signedTransaction = await this.tronWeb.trx.sign(transaction);
            const transactionResponse = await this.tronWeb.trx.sendRawTransaction(signedTransaction);
     ...
     * @param amount
     */
    async tokenTransaction(contract, from, to, privatekey, amount) {
        try {
            if (amount < 0) {
            ...
            const transferFunction = contractObj.methods.transfer(receiverAddress, amountInSUN);
            const options = {
                feeLimit: 100000000,
                callValue: 0,
                from: senderAddress,
            ...
            const tx = await this.tronWeb.transactionBuilder.triggerSmartContract(contract, "transfer(address,uint256)", options, [
...
exports.default = tronBlockChain;
