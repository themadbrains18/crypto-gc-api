...
class tradePairController extends main_controller_1.default {
    ...
    async allPairs(req, res, next) {
        try {
            let pairs = await service_1.default.pairServices.all();
            super.ok(res, pairs);
        }
        catch (error) {
            next(error);
    ...
    async allPairsByLimit(req, res, next) {
        try {
            ...
            let pairs = await service_1.default.pairServices.all();
            let pairsPaginate = await service_1.default.pairServices.allByLimit(offset, limit);
    ...
    async create(req, res, next) {
        try {
            ...
            //   }
            let tradePairExist = await models_1.tradePairModel.findOne({
            ...
            let pairResponse = await service_1.default.pairServices.create(trade);
            super.ok(res, {
    ...
    async activeInactivePair(req, res, next) {
        try {
            ...
            let data = { id, status };
            let statusResponse = await service_1.default.pairServices.changeStatus(data);
            if (statusResponse) {
                let trades = await service_1.default.pairServices.all();
    ...
    async edit(req, res, next) {
        try {
            ...
            // check token if already register
            //=======================================//
            let tradePairAlreadyRegister = await service_1.default.pairServices.alreadyExist(trade);
            if (tradePairAlreadyRegister) {
                let tradeResponse = await service_1.default.pairServices.edit(trade);
                if (tradeResponse) {
                    let trades = await service_1.default.pairServices.all();
                ...
                return super.fail(res, "Trade pair not registered. Please add new token.");
            }
        }
        catch (error) {
            console.log(error);
...
exports.default = tradePairController;
