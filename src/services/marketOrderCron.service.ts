...
class cronMarketOrderService {
    ...
    // async processLimitOrder(order: any) {
    //     return null
    // }

    // partial order code
    // async cronMarketPartialOrder(): Promise<any> {
    //     try {
    //         console.log('========here 1');
    //         let activeOrder = await marketOrderModel.findAll({ where: { status: false, isCanceled: false }, raw: true, order: [['createdAt', "DESC"]] });
    //         if (activeOrder) {
    //             //============================================================================//
    //             //=============Market Based spot market order partial execution===============//
    //             //============================================================================//
    //             let marketRequest = await activeOrder.filter((item) => {
    //                 return item.market_type === marektTypeEnum.market
    //             })
    //             console.log(marketRequest.length, '==============Market Request');
    //             if (marketRequest.length > 0) {
    //                 console.log('========here 2');
    //                 this.buySellOnMarket();
    //             }

    //             //===========================================================================//
    //             //=============Limit Based spot market order partial execution===============//
    //             //===========================================================================//
    //             let limitRequest = await activeOrder.filter((item: any) => {
    //                 return item?.market_type === marektTypeEnum.limit
    //             })
    //             console.log(limitRequest.length, '==============limit Request');
    ...
    //====================================================
    // async buySellOnLimit(): Promise<any> {
    //     try {
    //         await this.buyerCode();
    //         // await this.sellerCode();
    //     } catch (error: any) {
    ...
    async processBuyerExecution(options: buyerExecution) {
        try {
            ...
            if (sellerusdtmarket.status === false || sellerusdtmarket.status === 0) {
                ...
                if (asset) {
                    // let updatedBal2: any = truncateNumber(Number(parseFloat(asset.balance) + options.paid_usdt), 8);
                    // console.log(updatedBal2,'================updatedBal2');
                    let updatedBal: any = preciseAddition(parseFloat(asset.balance),options.paid_usdt);
            ...
            if (buyerusdtmarket?.status === false || buyerusdtmarket?.status === 0) {
                ...
                if (buyerasset) {
                    ...
                    let updatedBal: any = preciseAddition(parseFloat(buyerasset.balance),options.remainingAssets);
                    ...
                    if (options.remainingAssets > options.sellerObj.token_amount) {
                        ...
                        updatedBal = preciseAddition(parseFloat(buyerasset.balance),parseFloat(options.sellerObj.token_amount));
    ...
    //=====================================================
    // async buySellOnMarket(): Promise<any> {
    //     try {
    //         // await this.marketBuyerCode();
    //         // await this.marketSellerCode();
    //     } catch (error: any) {
    //         console.log(error?.message);
    //         console.log(error);
    //         throw new Error(error.message);
    ...
    /**
     * This code may be not required for market case
     */

    // async marketSellerCode(): Promise<any> {
    //     try {
    //         let previous_buyer: any = [];
    //         // if buyer not exist than return
    //         let buyBids = await marketOrderModel.findAll({ where: { status: false, isCanceled: false, order_type: marketOrderEnum.buy, market_type: marektTypeEnum.market, queue: false }, raw: true, order: [['id', "DESC"]] })
    //         if (buyBids == null || buyBids.length == 0) {
    //             throw new Error('No any buyer bids found');
    //         }
    //         // if seller not exist than return
    //         let sellBids = await marketOrderModel.findAll({ where: { status: false, isCanceled: false, order_type: marketOrderEnum.sell, market_type: marektTypeEnum.market, queue: false }, raw: true, order: [['id', "DESC"]] })
    //         if (sellBids == null || sellBids.length == 0) {
    //             throw new Error('No any seller bids found');
    //         }
    //         for await (const seller of sellBids) {
    //             let sellerObj = seller;
    //             let tokenFetch = await tokensModel.findOne({ where: { id: sellerObj.token_id }, raw: true });
    //             if (!tokenFetch) {
    //                 tokenFetch = await globalTokensModel.findOne({ where: { id: sellerObj.token_id }, raw: true });
    //             }
    ...
    /**
     * This code may be not required for limit case
     */

    // async sellerCode(): Promise<any> {
    //     try {
    //         let previous_buyer: any = [];
    //         // if buyer not exist than return
    //         let buyBids = await marketOrderModel.findAll({ where: { status: false, isCanceled: false, order_type: marketOrderEnum.buy, market_type: marektTypeEnum.limit, queue: false }, raw: true, order: [['createdAt', "DESC"]] })
    //         if (buyBids == null || buyBids.length == 0) {
    //             throw new Error('No any buyer bids found');
    //         }
    //         // if seller not exist than return
    //         let sellBids = await marketOrderModel.findAll({ where: { status: false, isCanceled: false, order_type: marketOrderEnum.sell, market_type: marektTypeEnum.limit, queue: false }, raw: true, order: [['createdAt', "DESC"]] })
    //         if (sellBids == null || sellBids.length == 0) {
    //             throw new Error('No any seller bids found');
    //         }
    //         for await (const seller of sellBids) {
