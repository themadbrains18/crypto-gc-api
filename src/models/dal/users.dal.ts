...
class userDal {
  ...
  checkUserReferCodeExist = async (refer: string): Promise<object | null> => {
    try {
      const user = await userModel.findOne({
        where: {
          [Op.and]: sequelize.where(fn('BINARY', col('own_code')), refer)
        }
        // where: fn('BINARY', col('own_code')), refer
      });
  ...
  checkUserByPk = async (id: string, t?: Transaction): Promise<UserOuput | any> => {
    const user = await userModel.findOne({
